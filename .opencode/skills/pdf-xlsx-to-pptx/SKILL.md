...
- **視覺美學**：插圖採用「古羅馬雕像日常風格 (Marmoreal Life)」，展現誇張表情與多元角色互動。
- **雙向對齊**：心智圖（須具備四層結構，第四層為頁碼，字體 10-12pt）與簡報內文必須 1:1 呼應，內文對應關鍵字需標示「黃色底色」。
- **層級比例**：心智圖 L2 (16pt) 必須大於 L3 (14pt)，確保視覺層次鮮明。
- **內容完整性**：重點完整保留，不進行過度精簡，雙語翻譯以括號 ( ) 簡略並置。
- **零殘缺佈局**：確保任何情況下不會發生疊字、疊圖、超出畫布的問題。
- **文件輸出**：輸出檔案存放在 `output/` 目錄中。
